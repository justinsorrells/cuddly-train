#!/usr/bin/env python3
"""
Agent Orchestrator for the Teensy 4.1 Command Server repo.
Orchestrates: Codex (implementation), Claude CLI (review), Antigravity (audit/coordination).

Ported from the special-lamp controller repo and adapted: checks run this
repo's verification suite (contract sync, invariants, host C++ tests, Teensy
compile), and forbidden-pattern scans enforce the firmware contract (board
statuses ok|error only, core/platform boundary, fixed-capacity JSON).
"""
...
class Orchestrator:
    def __init__(self, config: dict, dry_run: bool = False, allow_dirty: bool = False):
        ...
        self.run_outcomes: Dict[str, str] = {}
    ...
    def run_checks(self, changed_files: List[str]) -> Tuple[bool, Dict[str, str], str, List[str]]:
        """Runs configured checks (contract sync, invariants, host tests, Teensy build)."""
        ...
        fail_reason = ""
        failed_checks = []
        for name in ("compileall", "contract_sync", "invariants", "host_tests", "teensy_build"):
            self.run_outcomes.pop(name, None)

        # 1. compileall (only on changed Python files — orchestrator/tool maintenance)
        if checks_cfg.get("compileall", True):
            ...
            if not changed_py_files:
                ...
                self.run_outcomes["compileall"] = "SKIPPED"
            else:
                print(f"Compiling changed Python files: {changed_py_files}")
                code, out, err = self.run_cmd(["python3", "-m", "compileall"] + changed_py_files)
        ...
        # 2. Contract sync (hash-pinned contracts must not drift)
        if checks_cfg.get("contract_sync", True):
            print("Running contract sync check...")
            code, out, err = self.run_cmd(["python3", "tools/check_contract_sync.py"])
            results["contract_sync"] = out + "\n" + err
            self.log_artifact("contract_sync.txt", results["contract_sync"])
            if code != 0:
                passed_all = False
                failed_checks.append("contract_sync")
                if not fail_reason:
                    fail_reason = "STOP_CONTRACT_CHANGE"
                print("contract sync check failed!")
            else:
                print("contract sync check passed.")
        else:
            self.log_artifact("contract_sync.txt", "contract sync check was disabled.")

        # 3. Invariants checker
        if checks_cfg.get("invariants", True):
            print("Running invariants check...")
            code, out, err = self.run_cmd(["python3", "tools/check_invariants.py"])
        ...
        # 4. Host C++ tests
        if checks_cfg.get("host_tests", True):
            print("Running host C++ tests...")
            code, out, err = self.run_cmd(["./tools/run_host_tests.sh"])
            results["host_tests"] = out + "\n" + err
            self.log_artifact("host_tests.txt", results["host_tests"])
            if code != 0:
                passed_all = False
                failed_checks.append("host_tests")
                if not fail_reason:
                    fail_reason = "STOP_TESTS_FAILED"
                print(f"host tests failed with exit code {code}!")
            else:
                print("host tests passed.")
        else:
            self.log_artifact("host_tests.txt", "host tests were not configured.")

        # 5. Teensy compile (no-ops while no sketches exist)
        if checks_cfg.get("teensy_build", True):
            print("Running Teensy build...")
            code, out, err = self.run_cmd(["./tools/build_teensy.sh"])
            results["teensy_build"] = out + "\n" + err
            self.log_artifact("teensy_build.txt", results["teensy_build"])
            if code != 0:
                passed_all = False
                failed_checks.append("teensy_build")
                if not fail_reason:
                    fail_reason = "STOP_COMPILE_FAILED"
                print("Teensy build failed!")
            else:
                print("Teensy build passed.")
        else:
            self.log_artifact("teensy_build.txt", "Teensy build was disabled.")

        for name in ("compileall", "contract_sync", "invariants", "host_tests", "teensy_build"):
            if name in failed_checks:
                self.run_outcomes[name] = "FAIL"
            elif name in results:
                self.run_outcomes.setdefault(name, "PASS")
            else:
                self.run_outcomes[name] = "DISABLED"
    ...
    def check_forbidden_patterns(self, diff: str, changed_files: List[str], task_desc: str) -> Optional[str]:
        ...
        qnethernet_allowed_dirs = (
            "src/platform/qnethernet", "sketches", "tools", "docs", ".agents",
        )
        qnethernet_patterns = [
            r"#include\s*[<\"](QNEthernet|NativeEthernet|Ethernet)",
            r"\bEthernetClient\b",
            r"\bEthernetServer\b",
        ]
        ...
        for f in changed_files:
            ...
            in_src = f.startswith("src/")

            # Core/platform boundary: QNEthernet/Arduino networking types are
            # confined to src/platform/qnethernet and sketches/.
            f_in_allowed_dir = any(f == d or f.startswith(d + "/") for d in qnethernet_allowed_dirs)
            if not f_in_allowed_dir:
                for pattern in qnethernet_patterns:
                    if re.search(pattern, file_added):
                        return f"STOP_ARCHITECTURE_RISK (QNEthernet/Arduino networking outside src/platform/qnethernet: {f})"

            if in_src:
                # Fixed-capacity rule: no dynamic JSON documents (contract section 11).
                if re.search(r"\bDynamicJsonDocument\b", file_added):
                    return f"STOP_ARCHITECTURE_RISK (DynamicJsonDocument in {f}; fixed-capacity only)"
                # No Redis board-side, no board-level get_schema (contract sections 27, 9.3).
                if re.search(r"(?i)\bredis\b", file_added):
                    return f"STOP_ARCHITECTURE_RISK (Redis reference board-side in {f})"
                if re.search(r"['\"]get_schema['\"]", file_added):
                    return f"STOP_ARCHITECTURE_RISK (board-level get_schema in {f})"
                # Board statuses are exactly ok|error — never timeout, never new
                # values (contract section 16.4). Quoted protocol writes only
                # (doc["status"] = "x", "status":"x", doc[F("status")] = "x",
                # doc["status"].set("x"));
                # local members like state.status hold lifecycle values, not
                # wire statuses, and are not flagged.
                status_assign_matches = re.findall(
                    r"(?i)['\"]status['\"]\s*\)?\s*\]?\s*(?:[=:]|\.\s*set\s*\()\s*['\"]([a-zA-Z0-9_\-]+)['\"]",
                    file_added,
                )
                for stat in status_assign_matches:
                    if stat.lower() not in ["ok", "error"]:
                        return f"STOP_ARCHITECTURE_RISK (Forbidden board status in {f}: {stat})"
    ...
    def execute_task(self, task_file: Path) -> str:
        ...
        while cycle < max_cycles:
            ...
            self.final_artifacts["contract_sync.txt"] = check_logs.get("contract_sync", "")
            self.final_artifacts["compileall.txt"] = check_logs.get("compileall", "")
            self.final_artifacts["host_tests.txt"] = check_logs.get("host_tests", "")
            self.final_artifacts["teensy_build.txt"] = check_logs.get("teensy_build", "")
            ...
            if claude_verdict is None:
                self.run_outcomes["claude_review"] = "UNPARSEABLE"
            ...
            self.run_outcomes["claude_review"] = "FAIL" if (claude_verdict == "FAIL" or has_must_fix) else "PASS"
            ...
            antigravity_prompt = antigravity_template \
                ...
                .replace("{HOST_TEST_LOGS}", check_logs.get("host_tests", "No logs")) \
            ...
            if anti_verdict is None:
                self.run_outcomes["antigravity"] = "UNPARSEABLE"
            ...
            self.run_outcomes["antigravity"] = anti_verdict
            ...
            if anti_verdict == "PASS":
                ...
                tests_passed = (
                    "host_tests" not in failed_checks
                    and "compileall" not in failed_checks
                    and "teensy_build" not in failed_checks
                )
                invariants_passed = (
                    "invariants" not in failed_checks
                    and "contract_sync" not in failed_checks
                )
                ...
                command_path_files_changed = any(Path(f).stem in command_path_files for f in changed_files)
                
                safety_keywords = ["estop", "estop_ack", "blocked_by_estop", "safe state", "safety", "controller_loss", "on_controller_lost"]
                safety_or_estop_path_changed = any(kw in diff for kw in safety_keywords)
                
                seq_keywords = ["seq", "board_seq", "controller_ts"]
                seq_or_board_seq_logic_changed = any(kw in diff for kw in seq_keywords)
                
                writer_keywords = ["OutboundScheduler", "writeFully", "transmit deadline", "tx_deadline", "flush", "setNoDelay"]
                ...
                if adjudication_verdict is None:
                    self.run_outcomes["claude_adjudication"] = "UNPARSEABLE"
                ...
                self.run_outcomes["claude_adjudication"] = adjudication_verdict
    ...
    def log_report(self, task_summary: str, branch: str, classification: str, reason: str, commit_hash: Optional[str] = None, dry_run: Optional[bool] = None):
        ...
        # Verification outcomes come from recorded run facts, never inferred
        # from the terminal classification: a dry run reports NOT_RUN, and an
        # Antigravity FAIL that stops as STOP_HUMAN_REVIEW_REQUIRED still
        # reports FAIL.
        outcomes = getattr(self, "run_outcomes", {})

        def outcome(name: str) -> str:
            return outcomes.get(name, "NOT_RUN")
...
- **Contract Sync**: {outcome("contract_sync")}
- **Invariants**: {outcome("invariants")}
- **Host Tests**: {outcome("host_tests")}
- **Teensy Build**: {outcome("teensy_build")}
- **Compileall**: {outcome("compileall")}
- **Claude Review Verdict**: {outcome("claude_review")}
- **Claude Adjudication**: {outcome("claude_adjudication")}
- **Antigravity Audit Verdict**: {outcome("antigravity")}
